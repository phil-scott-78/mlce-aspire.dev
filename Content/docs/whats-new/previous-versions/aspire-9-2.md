...
If your AppHost project file doesn't have the `Aspire.AppHost.Sdk` reference, you might still be using Aspire 8. To upgrade to 9.0, follow [the upgrade guide](/docs/whats-new/upgrade-aspire).

## 🖥️ AppHost enhancements

The [AppHost](/docs/fundamentals/core-concepts/app-host) is the core of Aspire, providing the local hosting environment for your distributed applications. In Aspire 9.2, we've made several improvements to the AppHost:
...
For more information, see [Define custom resource URLs](/docs/extensibility/custom-resource-urls).
...
![Screenshot of dashboard resource's page showing the new resource icons.](/assets/whats-new/aspire-9.2/dashboard-resource-icons.png)
...
![Screenshot of the dashboard showing the pause button.](/assets/whats-new/aspire-9.2/dashboard-pause-telemetry.png)
...
![Screenshot of a metric with the cardinality limit warning.](/assets/whats-new/aspire-9.2/dashboard-cardinality-limit.png)
...
![Screenshot of console logs page showing the UTC timestamps option.](/assets/whats-new/aspire-9.2/dashboard-console-logs-utc.png)
...
[Custom resource commands](/docs/extensibility/custom-resource-commands) now support HTTP-based functionality with the addition of the `WithHttpCommand` API, enabling you to define endpoints for tasks like database migrations or resets. These commands can be run directly from the Aspire dashboard.
...
For more information, see [Custom HTTP commands in Aspire](/docs/extensibility/http-commands).
