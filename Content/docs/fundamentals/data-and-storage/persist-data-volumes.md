...
> [!NOTE]
> Volumes and bind mounts are features of your container runtime: Docker or
...
For the remainder of this article, imagine that you're exploring a `Program` class in an Aspire [AppHost project](/docs/fundamentals/core-concepts/app-host) that's already defined the distributed app builder bits:
...
> [!CAUTION]
> This isn't a persistent password! Instead, it changes every time the AppHost
>   runs.
...
The `AddParameter` method is used to create a parameter named `sql-password` that's considered a secret. The `AddSqlServer` method is then called with the `password` parameter to set the password for the SQL Server resource. For more information, see [External parameters](/docs/fundamentals/configuration/external-parameters).
